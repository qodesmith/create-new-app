...
The client never displays the server's error body — toasts use hardcoded client-side strings keyed off the action context. The response body is for debugging only. (This applies to the Hono RPC channel described here. Better Auth `authClient` calls are a separate channel that *does* conditionally surface server messages — see the "Better Auth Form Errors" section in [add-route](../add-route/SKILL.md).)
