...
Forms that call `authClient` methods (`signUp.email`, `changeEmail`, `changePassword`, `resetPassword`, `updateUser`, etc.) get errors from the **Better Auth client channel** — a separate channel from Hono RPC. Surface those errors with `getSafeAuthErrorMessage` from `@/client/lib/utils`, never raw `error.message`:

```ts
const {error} = await authClient.changePassword({...})
if (error) {
  toast.error(getSafeAuthErrorMessage(error, 'Failed to change password'))
  return
}
```

Why not `error.message ?? fallback`: a single endpoint returns BOTH our own validation errors AND library-generated ones (e.g. "User already exists"). Surfacing the raw message would leak library internals. `getSafeAuthErrorMessage` returns the server message only when it carries our `serverValidationErrorCode` marker, otherwise the fallback.

Server side (the coupled half): validation lives in the Better Auth `before` hook in `src/server/db/auth/auth.ts`. Any new validation must `throw new APIError('UNPROCESSABLE_ENTITY', {code: serverValidationErrorCode, message})` — the `UNPROCESSABLE_ENTITY` (422) status is the consistent code for validation failures, and the `serverValidationErrorCode` marker is what makes the message safe for the client to display.

Plain `onSubmitInvalid` / non-auth client validation still uses bare `toast.error('...')` strings as above; the helper is only for `authClient` call results.
